...
/**
 * Analysis and simulation frame for an individual trace.
 */
class TraceAnalysisFrame extends UIFrameTemplate {
    ...
    private void performAnalysis(String modifiedDeploymentModelSpec, DeploymentModel baseModel) {
        ...
        if (rewrittenTrace.size() < MAX_EVENTS_FOR_LIST) {
            var eventViews = rewrittenTrace.events().stream().map(EventView::new).sorted().collect(Collectors.toList());
    ...
    private static void createWriteChangeViews(Collection<EntityWriteEvent> events, WriteOutcome outcome, WriteChangeStatus status,
            Consumer<WriteChangeView> viewConsumer) {
    ...
    private record ConsistencyIssueView(long timestamp, ConsistencyIssueStatus status, String description, String affectedEntityType, String affectedEntityId)
            implements Comparable<ConsistencyIssueView> {
    ...
    private enum ConsistencyIssueStatus {
        NEW("new"), UNCHANGED("unchanged"), OBSOLETE("obsolete");
    ...
    private record WriteChangeView(long timestamp, WriteOutcome outcome, WriteChangeStatus status, String affectedEntityType, String affectedEntityId)
            implements Comparable<WriteChangeView> {
    ...
    private enum WriteOutcome {
        COMMITTED("committed"), REVERTED("reverted");
