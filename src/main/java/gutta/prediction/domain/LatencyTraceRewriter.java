...
public class LatencyTraceRewriter {
	...
	public List<MonitoringEvent> rewriteTrace(List<MonitoringEvent> events, Map<String, Component> useCaseAllocation, Map<String, Component> methodAllocation, ComponentConnections connections) {
	...
	private static class TraceRewriteWorker implements MonitoringEventVisitor<Void> {
		...
		public TraceRewriteWorker(List<MonitoringEvent> events, Map<String, Component> useCaseAllocation, Map<String, Component> methodAllocation, ComponentConnections connections) {
		...
		private void assertExpectedLocation(MonitoringEvent event) {			
			if (this.currentLocation != null && !this.currentLocation.equals(event.location())) {
				throw new IllegalStateException("Unexpected location at event '" + event + "': Expected '" + this.currentLocation + ", but found '" + event.location() + "'.");
			}
		}
		...
		public Void handleServiceCandidateInvocationEvent(ServiceCandidateInvocationEvent event) {
			...
			if (nextEvent instanceof ServiceCandidateEntryEvent entryEvent) {
				...
				var connection = this.connections.getConnection(sourceComponent, targetComponent).orElseThrow(() -> new IllegalStateException("No connection from '" + sourceComponent + "' to '" + targetComponent + "'."));
		...
		public Void handleTransactionAbortEvent(TransactionAbortEvent event) {
			...
			var rewrittenEvent = new TransactionAbortEvent(event.traceId(), this.adjustTimestamp(event.timestamp()), event.location(), event.transactionId(), event.cause());
			return this.addRewrittenEvent(rewrittenEvent);
		}
		...
		public Void handleEntityReadEvent(EntityReadEvent event) {
			...
			var rewrittenEvent = new EntityReadEvent(event.traceId(), this.adjustTimestamp(event.timestamp()), event.location(), event.entityType(), event.entityIdentifier());
			return this.addRewrittenEvent(rewrittenEvent);
		}
		...
		public Void handleEntityWriteEvent(EntityWriteEvent event) {
			...
			var rewrittenEvent = new EntityWriteEvent(event.traceId(), this.adjustTimestamp(event.timestamp()), event.location(), event.entityType(), event.entityIdentifier());
			return this.addRewrittenEvent(rewrittenEvent);
		}
